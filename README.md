...
### AI Analyze
현재 활성화된 코드 경로(Collision/Memory/Spawn 조합)의 최근 30초 텔레메트리를 서버가 결정론적으로 계산한 뒤, 이를 근거로 LLM이 병목 원인을 해석해 보여줍니다. 결과는 마크다운이 렌더링된 형태(병목 진단 / 최적화된 C++ 스니펫 / 예상 효과)로 표시되며, 화면 우측 "Active Code Path" 패널의 **AI Analyze** 버튼으로 실행하거나 별도 팝업 창으로도 볼 수 있습니다.
