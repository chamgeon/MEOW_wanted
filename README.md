...
화면 하단의 **Logger — 1 Hz** 패널은 초당 한 번 스냅샷을 기록합니다: fps, sim/frame의 mean·p95·p99·max, 엔티티 수, game speed, kills/s, 그리고 **over budget**(프레임 예산을 초과한 프레임 수 / 전체 프레임 수)까지 보여줍니다. **Download .ndjson** 버튼으로 기록된 로그 전체를 파일로 받을 수 있습니다.
...
### AI Analyze
현재 활성화된 코드 경로(Collision/Memory/Spawn 조합)의 최근 30초 텔레메트리를 서버가 결정론적으로 계산한 뒤, 이를 근거로 LLM이 병목 원인을 해석해 보여줍니다. 결과는 마크다운이 렌더링된 형태(병목 진단 / 최적화된 C++ 스니펫 / 예상 효과 / 성능 저하 예측)로 표시되며, 화면 우측 "Active Code Path" 패널의 **AI Analyze** 버튼으로 실행하거나 별도 팝업 창으로도 볼 수 있습니다.

### Optimize
현재 설정(Collision/Memory/Spawn/Speed)을 기준으로, 대안 알고리즘 3개 × AoS/SoA 2개 = **총 6개 조합**을 실제 WASM 바이너리로 빌드해 동일 시드로 벤치마크 비교합니다. 모델이 임의로 C++ 코드를 생성하지 않고, 검증된 후보 생성기만 사용합니다. 가장 빠른 추천 조합은 신뢰도(Confidence) 표시와 함께 강조되어 나타나며, 최소 3개 이상의 성능 로그 구간이 쌓여야 실행 가능합니다.
